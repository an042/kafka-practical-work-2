    ...
    while True:
        ...
        if msg.error(): # обработка ошибок
            if msg.error().code() == KafkaException._PARTITION_EOF:
                ...
                with open("./kafka_yandex/Module1/logfile_BatchMessageConsumer.txt", "a", encoding="utf-8") as file_test:
                    print(f"[{consumer_id}] Ошибка сообщения: {msg.error()}", file=file_test)
                    file_test.close()
                break
        batch.append(msg) # Добавления в batch элемента msg с помощью метода append()
        if len(batch) >= 10: # Обработка при получении 10 сообщений
            ...
            batch.clear() # Eдаляет все его элементы, делая список пустым.
            
except KeyboardInterrupt:
    print(f"[{consumer_id}] Прерывание пользователем")
    # Добавление ошибки в лог
    with open("./kafka_yandex/Module1/logfile_BatchMessageConsumer.txt", "a", encoding="utf-8") as file_test:
        print(f"[{consumer_id}] Прерывание пользователем", file=file_test)
        file_test.close()
finally:
    consumer.close()
